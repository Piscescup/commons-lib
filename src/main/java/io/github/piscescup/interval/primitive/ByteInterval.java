...
/**
 * A primitive {@code byte}-based interval with explicit endpoint inclusiveness.
 *
 ...
 * </ul>
 *
 * <p>Ordering is the natural numeric ordering of {@code byte}. Therefore, the
 ...
 * <p>This class follows the common interval convention:
 * when {@code minimum == maximum}, the interval is empty unless it is closed on
 ...
 * ByteInterval closed    = ByteInterval.of((byte) 1, (byte) 3, IntervalType.CLOSED_INTERVAL);       // [1, 3]
 * ByteInterval open      = ByteInterval.of((byte) 1, (byte) 3, IntervalType.OPEN_INTERVAL);         // (1, 3)
 * ByteInterval openEnd   = ByteInterval.of((byte) 1, (byte) 3, IntervalType.CLOSED_OPEN_INTERVAL);  // [1, 3)
 * ByteInterval openStart = ByteInterval.of((byte) 1, (byte) 3, IntervalType.OPEN_CLOSED_INTERVAL);  // (1, 3]
 *
 * // Membership
 * closed.contains((byte) 1); // true  (start inclusive)
 * open.contains((byte) 1);   // false (start exclusive)
 *
 * // Degenerate / empty
 * ByteInterval pointClosed = ByteInterval.of((byte) 5, (byte) 5, IntervalType.CLOSED_INTERVAL); // [5, 5]
 * pointClosed.isEmpty();      // false
 * pointClosed.isDegenerate(); // true
 * pointClosed.contains((byte) 5); // true
 *
 * ByteInterval pointOpen = ByteInterval.of((byte) 5, (byte) 5, IntervalType.OPEN_INTERVAL); // (5, 5)
 * pointOpen.isEmpty(); // true
 *
 * // Overlap and intersection
 * ByteInterval a = ByteInterval.of((byte) 1, (byte) 3, IntervalType.CLOSED_INTERVAL); // [1, 3]
 * ByteInterval b = ByteInterval.of((byte) 3, (byte) 8, IntervalType.CLOSED_INTERVAL); // [3, 8]
 * a.overlaps(b); // true (touch at 3 and both inclusive at the touching endpoints)
 *
 * Optional<ByteInterval> inter = a.intersection(b); // Optional[[3, 3]]
...
public class ByteInterval implements PrimitiveInterval, Serializable {
    ...
    /**
     * Creates a {@code ByteInterval} with the given bounds and endpoint type.
     *
     * <p>The interval is valid only when {@code minimum <= maximum}.
     *
     * <pre>{@code
     * ByteInterval i = ByteInterval.of((byte) 1, (byte) 10, IntervalType.CLOSED_OPEN_INTERVAL); // [1, 10)
     * }</pre>
     *
     * @param minimum      the start bound
     * @param maximum      the end bound
     * @param intervalType the endpoint inclusiveness descriptor
     * @return a new {@code ByteInterval}
     ...
     */
    @Contract("_, _, _ -> new")
    public static @NotNull ByteInterval of(byte minimum, byte maximum, IntervalType intervalType) {
        NullCheck.requireNonNull(intervalType);
        if (minimum > maximum) {
            throw new IllegalMathIntervalEndpointException(minimum, maximum);
        }
        return new ByteInterval(minimum, maximum, intervalType);
    }

    /**
     * Constructs a new {@code ByteInterval}.
     *
     * <p>Callers should prefer {@link #of(byte, byte, IntervalType)} which performs
     * validation.
     *
     * @param minimum      the start bound
     * @param maximum      the end bound
     * @param intervalType the endpoint inclusiveness descriptor
     */
    private ByteInterval(byte minimum, byte maximum, IntervalType intervalType) {
     ...
     */
    public @NotNull Comparator<Byte> getComparator() {
     ...
     *
     * <pre>{@code
     * ByteInterval i = ByteInterval.of((byte) 1, (byte) 3, IntervalType.CLOSED_OPEN_INTERVAL); // [1, 3)
     * i.onStartEndpoint((byte) 1); // true
     * i.onStartEndpoint((byte) 2); // false
     * }</pre>
     *
     * @param value the value to test
     * @return {@code true} if {@code value == minimum} and start is inclusive
     */
    public boolean onStartEndpoint(byte value) {
     ...
     */
    public boolean onEndEndpoint(byte value) {
     ...
     *
     * <pre>{@code
     * ByteInterval i = ByteInterval.of((byte) 1, (byte) 3, IntervalType.OPEN_CLOSED_INTERVAL); // (1, 3]
     * i.contains((byte) 1); // false
     * i.contains((byte) 2); // true
     * i.contains((byte) 3); // true
     * }</pre>
     *
     * @param value the value to test
    ...
    public boolean contains(byte value) {
        int cs = Byte.compare(value, minimum);
        int ce = Byte.compare(value, maximum);
     ...
     *
     * <pre>{@code
     * ByteInterval outer = ByteInterval.of((byte) 1, (byte) 10, IntervalType.CLOSED_INTERVAL); // [1,10]
     * ByteInterval inner = ByteInterval.of((byte) 3, (byte) 7, IntervalType.OPEN_INTERVAL);   // (3,7)
     ...
     */
    public boolean containsInterval(ByteInterval other) {
        if (other == null) return false;

        // start bound
        int startCmp = Byte.compare(other.minimum, this.minimum);
        boolean startOk =
            startCmp > 0 ||
                (startCmp == 0 && (!other.isStartInclusive() || isStartInclusive()));

        // end bound
        int endCmp = Byte.compare(other.maximum, this.maximum);
        boolean endOk =
            endCmp < 0 ||
     ...
     */
    public boolean isContainedBy(ByteInterval other) {
        if (other == null) return false;
     ...
     *
     * <pre>{@code
     * ByteInterval a = ByteInterval.of((byte) 5, (byte) 5, IntervalType.CLOSED_INTERVAL); // [5,5]
     * a.isEmpty(); // false
     *
     * ByteInterval b = ByteInterval.of((byte) 5, (byte) 5, IntervalType.OPEN_CLOSED_INTERVAL); // (5,5]
     * b.isEmpty(); // true
     * }</pre>
     *
    ...
    public boolean isEmpty() {
        int c = Byte.compare(minimum, maximum);
        if (c > 0) return true;
        if (c < 0) return false;
     ...
     * a.overlaps(b); // true
     *
     * ByteInterval c = ByteInterval.of((byte) 1, (byte) 3, IntervalType.CLOSED_OPEN_INTERVAL); // [1,3)
     * ByteInterval d = ByteInterval.of((byte) 3, (byte) 8, IntervalType.CLOSED_INTERVAL);      // [3,8]
     ...
     */
    public boolean overlaps(ByteInterval other) {
        if (other == null) return false;
        ...
        int sCmp = Byte.compare(this.minimum, other.maximum);
        if (sCmp > 0) return false;
        if (sCmp == 0 && !(this.isStartInclusive() && other.isEndInclusive())) return false;

        int eCmp = Byte.compare(this.maximum, other.minimum);
     ...
     *
     * <pre>{@code
     * ByteInterval a = ByteInterval.of((byte) 1, (byte) 5, IntervalType.CLOSED_INTERVAL); // [1,5]
     * ByteInterval b = ByteInterval.of((byte) 3, (byte) 8, IntervalType.OPEN_CLOSED_INTERVAL); // (3,8]
     * Optional<ByteInterval> inter = a.intersection(b); // (3,5]
     * }</pre>
     *
     * @param other the other interval
     * @return an {@code Optional} describing the intersection, if present
     */
    public Optional<ByteInterval> intersection(ByteInterval other) {
        ...
        byte newStart;
        boolean newStartInc;
        int startCmp = Byte.compare(this.minimum, other.minimum);
        if (startCmp > 0) {
        ...
        byte newEnd;
        boolean newEndInc;
        int endCmp = Byte.compare(this.maximum, other.maximum);
        if (endCmp < 0) {
            newEnd = this.maximum;
        ...
        return Optional.of(ByteInterval.of(newStart, newEnd, type));
    }
     ...
     * ByteInterval i = ByteInterval.of((byte) 1, (byte) 3, IntervalType.OPEN_CLOSED_INTERVAL);
     * i.startsAfter((byte) 1); // true
     * }</pre>
     *
     ...
     */
    public boolean startsAfter(byte value) {
        int comparison = Byte.compare(this.minimum, value);
     ...
     */
    public boolean startsAfterStrictly(byte value) {
        int cmp = Byte.compare(this.minimum, value);
     ...
     */
    public boolean endsBefore(byte value) {
        int comparison = Byte.compare(maximum, value);
     ...
     */
    public boolean endsBeforeStrictly(byte value) {
        int cmp = Byte.compare(maximum, value);
     ...
     *
     * <pre>{@code
     * ByteInterval i = ByteInterval.of((byte) 1, (byte) 3, IntervalType.CLOSED_OPEN_INTERVAL);
    ...
    /**
     * Returns an empty {@link Byte} {@link ObjectInterval} instance.
     *
