...
import io.github.piscescup.interfaces.exfunction.primitive.FloatFunction;
 ...
 * whose functional method is {@link #apply(Object, float)}.
 *
 ...
 */
@FunctionalInterface
public interface ObjFloatToObjFunction<X, Y> {
     ...
     */
    default <V> ObjFloatToObjFunction<X, V> andThen(
     ...
     */
    default Function<X, FloatFunction<Y>> curried() {
     ...
     */
    default ObjFloatToObjFunction<X, Y> memorized() {
        if (isMemorized()) return this;

        final Map<BinEntry<X, Float>, Y> cache = new ConcurrentHashMap<>();
        return (ObjFloatToObjFunction<X, Y> & Memorized) (x, value) -> {
     ...
     */
    static <X, Y> ObjFloatToObjFunction<X, Y> constant(Y constant) {
     ...
     */
    static <X, Y> ObjFloatToObjFunction<X, Y> of(
        ObjFloatToObjFunction<X, Y> function
    ) {
    ...
    static <X, Y> ObjFloatToObjFunction<X, Y> narrow(
        ObjFloatToObjFunction<? super X, ? extends Y> function
    ) {
        NullCheck.requireNonNull(function);
        return (ObjFloatToObjFunction<X, Y>) function;
    }
}
