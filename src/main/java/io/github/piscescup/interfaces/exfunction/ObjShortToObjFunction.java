...
import io.github.piscescup.interfaces.exfunction.primitive.ShortFunction;
 ...
 * whose functional method is {@link #apply(Object, short)}.
 *
 ...
 */
@FunctionalInterface
public interface ObjShortToObjFunction<X, Y> {
     ...
     */
    default <V> ObjShortToObjFunction<X, V> andThen(
     ...
     */
    default Function<X, ShortFunction<Y>> curried() {
     ...
     */
    default ObjShortToObjFunction<X, Y> memorized() {
        if (isMemorized()) return this;

        final Map<BinEntry<X, Short>, Y> cache = new ConcurrentHashMap<>();
        return (ObjShortToObjFunction<X, Y> & Memorized) (x, value) -> {
     ...
     */
    static <X, Y> ObjShortToObjFunction<X, Y> constant(Y constant) {
     ...
     */
    static <X, Y> ObjShortToObjFunction<X, Y> of(
        ObjShortToObjFunction<X, Y> function
    ) {
    ...
    static <X, Y> ObjShortToObjFunction<X, Y> narrow(
        ObjShortToObjFunction<? super X, ? extends Y> function
    ) {
        NullCheck.requireNonNull(function);
        return (ObjShortToObjFunction<X, Y>) function;
    }
}
