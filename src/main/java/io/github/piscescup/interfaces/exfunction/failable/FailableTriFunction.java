 ...
 * {@link #applyOrThrow(Object, Object, Object)}.
 *
 ...
 */
@FunctionalInterface
public interface FailableTriFunction<X1, X2, X3, Y, E extends Throwable> {
     ...
     */
    Y applyOrThrow(X1 x1, X2 x2, X3 x3) throws E;

    /**
     * Returns a composed {@code FailableTriFunction} that first applies this function
     ...
     * @return a composed {@code FailableTriFunction} that first applies this function
     *         and then applies the {@code after} function
     * @throws NullPointerException if {@code after} is {@code null}
     */
    default <V> FailableTriFunction<X1, X2, X3, V, E> andThen(
        Function<? super Y, ? extends V> after
    ) {
        NullCheck.requireNonNull(after);
        return (x1, x2, x3) -> after.apply(this.applyOrThrow(x1, x2, x3));
    }
    ...
    default <V> FailableTriFunction<X1, X2, X3, V, E> andThen(
        ...
        NullCheck.requireNonNull(after);
        return (x1, x2, x3) -> after.applyOrThrow(this.applyOrThrow(x1, x2, x3));
    }

    /**
     * Narrows a {@code FailableTriFunction} with broader generic bounds to a more
     * specific type.
     * <p>
    ...
    static <X1, X2, X3, Y, E extends Throwable> FailableTriFunction<X1, X2, X3, Y, E> narrow(
        FailableTriFunction<? super X1, ? super X2, ? super X3, ? super Y, ? super E> function
    ) {
        NullCheck.requireNonNull(function);
        return (FailableTriFunction<X1, X2, X3, Y, E>) function;
    }

    /**
     * Returns a {@code FailableTriFunction} that always returns the given constant value.
     *
     ...
     */
    static <X1, X2, X3, Y, E extends Throwable> @NotNull FailableTriFunction<X1, X2, X3, Y, E> constant(
        Y constant
    ) {
        NullCheck.requireNonNull(constant);
        return (x1, x2, x3) -> constant;
    }
}
