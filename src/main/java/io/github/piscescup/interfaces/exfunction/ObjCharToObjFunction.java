...
import io.github.piscescup.interfaces.exfunction.primitive.CharFunction;
 ...
 * whose functional method is {@link #apply(Object, char)}.
 *
 ...
 */
@FunctionalInterface
public interface ObjCharToObjFunction<X, Y> {
     ...
     */
    default <V> ObjCharToObjFunction<X, V> andThen(
     ...
     */
    default Function<X, CharFunction<Y>> curried() {
     ...
     */
    default ObjCharToObjFunction<X, Y> memorized() {
        if (isMemorized()) return this;

        final Map<BinEntry<X, Character>, Y> cache = new ConcurrentHashMap<>();
        return (ObjCharToObjFunction<X, Y> & Memorized) (x, value) -> {
     ...
     */
    static <X, Y> ObjCharToObjFunction<X, Y> constant(Y constant) {
     ...
     */
    static <X, Y> ObjCharToObjFunction<X, Y> of(
        ObjCharToObjFunction<X, Y> function
    ) {
    ...
    static <X, Y> ObjCharToObjFunction<X, Y> narrow(
        ObjCharToObjFunction<? super X, ? extends Y> function
    ) {
        NullCheck.requireNonNull(function);
        return (ObjCharToObjFunction<X, Y>) function;
    }
}
