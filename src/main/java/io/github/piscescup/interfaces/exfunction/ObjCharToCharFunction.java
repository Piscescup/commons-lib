...
import io.github.piscescup.interfaces.exfunction.primitive.CharUnaryOperator;
 ...
 * {@code char}-valued argument and produces a {@code char}-valued
 * result.
 *
 * <p>This is a <a href="package-summary.html">functional interface</a>
 * whose functional method is {@link #applyAsChar(Object, char)}.
 *
 ...
 */
@FunctionalInterface
public interface ObjCharToCharFunction<X> {
     ...
     */
    char applyAsChar(X x, char value);

    /**
     * Applies this function and returns its result as a {@link Character}.
     *
     * @param x the object argument
     * @param value the {@code char}-valued argument
     ...
     */
    default Character boxedApply(X x, char value) {
        NullCheck.requireNonNull(x);
        return applyAsChar(x, value);
    }
     ...
     * @throws NullPointerException if {@code x} is {@code null}
     */
    default CharUnaryOperator apply(X x) {
        NullCheck.requireNonNull(x);
        return value -> applyAsChar(x, value);
    }
     ...
     */
    default ObjCharToCharFunction<X> andThen(CharUnaryOperator after) {
        NullCheck.requireNonNull(after);
        return (x, value) -> after.applyAsChar(applyAsChar(x, value));
    }
     ...
     */
    default Function<X, CharUnaryOperator> curried() {
        return x -> value -> applyAsChar(x, value);
    }
     ...
     */
    default ObjCharToCharFunction<X> memorized() {
        if (isMemorized()) return this;

        final Map<BinEntry<X, Character>, Character> cache =
            new ConcurrentHashMap<>();

        return (ObjCharToCharFunction<X> & Memorized) (x, value) -> {
            final BinEntry<X, Character> entry = new BinEntry<>(x, value);
            return cache.computeIfAbsent(
                entry,
                e -> applyAsChar(e.x1(), e.x2())
            );
        };
    }
     ...
     */
    static <X> ObjCharToCharFunction<X> constant(char constant) {
     ...
     */
    static <X> ObjCharToCharFunction<X> of(ObjCharToCharFunction<X> function) {
    ...
    static <X> ObjCharToCharFunction<X> narrow(
        ObjCharToCharFunction<? super X> function
    ) {
        NullCheck.requireNonNull(function);
        return (ObjCharToCharFunction<X>) function;
    }
}
