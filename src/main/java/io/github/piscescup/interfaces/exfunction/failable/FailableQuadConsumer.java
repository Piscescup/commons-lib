...
import io.github.piscescup.interfaces.exfunction.QuadConsumer;
 ...
 * {@link #acceptOrThrow(Object, Object, Object, Object)}.
 *
 ...
 */
@FunctionalInterface
public interface FailableQuadConsumer<X1, X2, X3, X4, E extends Throwable> {
     ...
     */
    void acceptOrThrow(X1 x1, X2 x2, X3 x3, X4 x4) throws E;

    /**
     * Returns a composed {@code FailableQuadConsumer} that performs, in sequence,
     ...
     * @return a composed {@code FailableQuadConsumer} that performs in sequence
     *         this operation followed by the {@code after} operation
     * @throws NullPointerException if {@code after} is {@code null}
     */
    default FailableQuadConsumer<X1, X2, X3, X4, E> andThen(
        FailableQuadConsumer<? super X1, ? super X2, ? super X3, ? super X4, E> after
    ) {
        NullCheck.requireNonNull(after);
        return (x1, x2, x3, x4) -> {
            acceptOrThrow(x1, x2, x3, x4);
            after.acceptOrThrow(x1, x2, x3, x4);
        };
    }
     ...
     */
    default FailableQuadConsumer<X1, X2, X3, X4, E> andThen(
        QuadConsumer<? super X1, ? super X2, ? super X3, ? super X4> after
    ) {
        NullCheck.requireNonNull(after);
        return (x1, x2, x3, x4) -> {
            acceptOrThrow(x1, x2, x3, x4);
    ...
    /**
     * Returns a {@code FailableQuadConsumer} that performs no operation.
     *
     ...
     */
    @Contract(pure = true)
    static <X1, X2, X3, X4, E extends Throwable> @NotNull FailableQuadConsumer<X1, X2, X3, X4, E> empty() {
        return (x1, x2, x3, x4) -> {};
    }

    /**
     * Narrows a {@code FailableQuadConsumer} with broader generic bounds to a more
     * specific type.
     * <p>
    ...
    static <X1, X2, X3, X4, E extends Throwable> FailableQuadConsumer<X1, X2, X3, X4, E> narrow(
        FailableQuadConsumer<? super X1, ? super X2, ? super X3, ? super X4, ? super E> consumer
    ) {
        NullCheck.requireNonNull(consumer);
        return (FailableQuadConsumer<X1, X2, X3, X4, E>) consumer;
    }
}
