 ...
 * whose functional method is {@link #test(int, int, int, int, int)}.</p>
 *
 * @author REN YuanTong
 * @since 1.1.0
 */
@FunctionalInterface
public interface IntQuinPredicate {
     ...
     */
    boolean test(int x1, int x2, int x3, int x4, int x5);
     ...
     */
    default IntQuinPredicate and(IntQuinPredicate other) {
     ...
     */
    default IntQuinPredicate negate() {
        return (x1, x2, x3, x4, x5) ->
            !test(x1, x2, x3, x4, x5);
    }
     ...
     * @throws NullPointerException if {@code other} is {@code null}
     */
    default IntQuinPredicate or(IntQuinPredicate other) {
        NullCheck.requireNonNull(other);
        return (x1, x2, x3, x4, x5) ->
            test(x1, x2, x3, x4, x5) || other.test(x1, x2, x3, x4, x5);
    }
