 ...
 */
@FunctionalInterface
public interface ObjIntToObjFunction<X, Y> {
     ...
     */
    default <V> ObjIntToObjFunction<X, V> andThen(
     ...
     */
    default Function<X, IntFunction<Y>> curried() {
     ...
     */
    default ObjIntToObjFunction<X, Y> memorized() {
        if (isMemorized()) return this;

        final Map<BinEntry<X, Integer>, Y> cache = new ConcurrentHashMap<>();
        return (ObjIntToObjFunction<X, Y> & Memorized) (x, value) -> {
     ...
     */
    static <X, Y> ObjIntToObjFunction<X, Y> constant(Y constant) {
     ...
     */
    static <X, Y> ObjIntToObjFunction<X, Y> of(
        ObjIntToObjFunction<X, Y> function
    ) {
    ...
    static <X, Y> ObjIntToObjFunction<X, Y> narrow(
        ObjIntToObjFunction<? super X, ? extends Y> function
    ) {
        NullCheck.requireNonNull(function);
        return (ObjIntToObjFunction<X, Y>) function;
    }
}
